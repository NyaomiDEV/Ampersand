...
import { defaultCustomFront, defaultMember } from "../tables/members";
...
function tag(spExport: any){
	...
	for (const spGroup of spExport.groups) {
		...
		tags.push(tag);
		tagMapping.set(spGroup._id, tag);
	}
...
function customField(spExport: any){
	const customFieldMapping = new Map<string, [CustomField, number]>();
	...
	for (const spCustomField of spExport.customFields) {
		...
		customFieldMapping.set(spCustomField._id, [customField, spCustomField.type]);
	}
...
export async function member(spExport: any, systemInfo: System, systemUid: string, tagMapping: Map<string, Tag>, customFieldMapping: Map<string, [CustomField, number]>){
	...
	for (const spMember of spExport.members) {
		...
		const member: Member = {
			...
			customFields: new Map(Object.entries(spMember.info || {})
				.map(([_id, value]) => {
					if (!(value as string).length) return undefined;
					const mapped = customFieldMapping.get(_id);
					if (!mapped) return undefined;

					return [mapped[0], mapCustomFieldType(mapped[1], value)] as [CustomField, string];
				})
				.filter(x => !!x)),
			id: window.crypto.randomUUID()
		};
		...
		members.push(member);
		memberMapping.set(spMember._id, member);
		console.debug("[SP] Created member for:", spMember, "with UUID:", member.id);
	}
	// END MEMBERS
	...
	for (const spCustomFront of spExport.frontStatuses) {
		...
		members.push(member);
		memberMapping.set(spCustomFront._id, member);
		console.debug("[SP] Created member for custom front:", spCustomFront, "with UUID:", member.id);
	}
	// END CUSTOM FRONTS
...
function frontingEntry(spExport: any, memberMapping: Map<string, Member>){
	...
	for (const spFrontHistory of spExport.frontHistory) {
		...
		const frontingEntry: FrontingEntry = {
			member: memberMapping.get(spFrontHistory.member) || (spFrontHistory.custom ? defaultCustomFront() : defaultMember()),
		...
		console.debug("[SP] Created fronting entry for:", spFrontHistory, "with UUID:", frontingEntry.id);
	}

	return frontingEntries;
}

function boardMessage(spExport: any, memberMapping: Map<string, Member>){
	...
	for (const spBoardMessage of spExport.boardMessages) {
		...
		const boardMessage: BoardMessage = {
			member: memberMapping.get(spBoardMessage.writtenBy) || defaultMember(),
			title: spBoardMessage.title,
			body: `@<m:${memberMapping.get(spBoardMessage.writtenFor)!.id}>\n\n${spBoardMessage.message?.length ? spBoardMessage.message : ""}`,
		...
		console.debug("[SP] Created message board entry for:", spBoardMessage, "with UUID:", boardMessage.id);
	}
	// END BOARD MESSAGES
	...
	for (const spPoll of spExport.polls) {
		...
		boardMessages.push(boardMessage);
		console.debug("[SP] Created message board entry for poll:", spPoll, "with UUID:", boardMessage.id);
	}
	// END POLLS
...
function journalPost(spExport: any, memberMapping: Map<string, Member>){
	...
	for(const spNote of spExport.notes){
		const post: JournalPost = {
			title: spNote.title || "--",
			body: spNote.note,
			member: spNote.member ? memberMapping.get(spNote.member) || defaultMember() : defaultMember(),
			tags: [],
			isPrivate: false,
			isPinned: false,
...
function remap(
	memberMapping: Map<string, Member>,
	systemInfo: System,
	members: Member[],
	...
	if (systemInfo.description)
		systemInfo.description = systemInfo.description.replace(/<###@(\w+)###>/g, (_, p1) => `@<m:${memberMapping.get(p1)!.id || nilUid}>`);

	for (const member of members) {
		if (member.description)
			member.description = member.description.replace(/<###@(\w+)###>/g, (_, p1) => `@<m:${memberMapping.get(p1)!.id || nilUid}>`);

		if (member.customFields)
			member.customFields = new Map(Array.from(member.customFields.entries()).map(([k, v]) => [k, v.replace(/<###@(\w+)###>/g, (_, p1) => `@<m:${memberMapping.get(p1)!.id || nilUid}>`)]));
	}

	for (const boardMessage of boardMessages) {
		if (boardMessage.body)
			boardMessage.body = boardMessage.body.replace(/<###@(\w+)###>/g, (_, p1) => `@<m:${memberMapping.get(p1)!.id || nilUid}>`);
	}

	for (const post of posts) 
		post.body = post.body.replace(/<###@(\w+)###>/g, (_, p1) => `@<m:${memberMapping.get(p1)!.id || nilUid}>`);
