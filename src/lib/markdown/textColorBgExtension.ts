			...
			renderer(token) {
				...
				if(colors.length){
					const cssStyle: Record<string, string> = {
						...
						"--markdown-text-bg-degrees": directions.includes(token.linearDegrees) ? `to ${token.linearDegrees}` : token.linearDegrees,
						"--markdown-text-bg-radial-definition": token.radialDefinition
					};

					return h("span", {
						class: ["text-color-bg", token.isRepeating ? "repeating" : undefined, token.isRadial ? "radial" : undefined],
						style: cssStyle
