<script setup lang="ts">
	...
	import type { Member, Comment, FrontingEntryComplete } from "../lib/db/entities";
	import { updateFrontingEntry } from "../lib/db/tables/frontingEntries.ts";
	...
	async function getCommentMembers(){
		const commentMemberUUIDs = entry.value.comments?.map(x => x.member) || [];
	...
	async function addComment(){
		...
		entry.value.comments?.push(comment);

		await updateFrontingEntry({ ...toRaw(entry.value), member: entry.value.member.uuid, influencing: entry.value.influencing?.uuid });
	}

	async function deleteComment(comment: Comment){
		closeSlidingItems();
		if(await promptOkCancel(i18next.t("other:comments.deleteConfirmation"))){
			entry.value.comments?.splice(
				entry.value.comments.indexOf(comment),
				1
			);
			await updateFrontingEntry({ ...toRaw(entry.value), member: entry.value.member.uuid, influencing: entry.value.influencing?.uuid });
		}
...
<template>
	<IonModal class="poll-results-modal" :breakpoints="[0,0.75,1]" initial-breakpoint="1">
		...
		<SpinnerFullscreen v-if="!members || !entry.comments" />
		<IonContent v-else>
			<IonList ref="list">
				<VirtualList :entries="entry.comments.toSorted(sortDateAsc)" :gap="8" :min-size="92">
