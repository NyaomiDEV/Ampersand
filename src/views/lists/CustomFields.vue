<script setup lang="ts">
	...
	async function saveFilterQuery(){
		...
		const vnode = h(FilterQueryEdit, {
			filterQuery: { name: "", type: "customFields", query: search.value },
			onDidDismiss: () => removeModal(vnode)
		});
		...
		<IonContent v-else>
			<CollapsibleHeaderbar class="size-large">
				...
				<IonToolbar>
					...
					/>
					<IonButtons slot="end">
						<IonButton v-if="getFilterQueriesIndex().filter(x => x.type === 'customFields').length" @click="filterQuerySelect?.$el.present()">
