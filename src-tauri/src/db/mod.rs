...
use crate::{Error, db::types::msgpack};
...
pub fn init(app: &App) -> crate::Result<()> {
	let db_path = &app.path().app_data_dir()?.join("db.sqlite");
	let db = Connection::open(db_path)?;
    match DATABASE.set(Mutex::new(db)) {
		Ok(()) => Ok(()),
		Err(val) => Err(Error::DatabaseInitializedError(val))
	}
}

pub fn db_test() -> crate::Result<String> {
	Ok(DATABASE
		.get()
		.ok_or(Error::Poison)? // seems appropriate?
		.lock()?
...
pub fn db_run_migrations<R: Runtime>(
	...
	let mut migrations = handle.ampersand().list_assets(MIGRATIONS_PATH.to_string())?;
	migrations.sort();
...
pub fn db_migrate_old<R: Runtime>(
	...
	let conn = DATABASE.get().ok_or(Error::Poison)?; // seems appropriate?
